       ...
       /* alert('*********** correct ************');            isNaN(pnrno) ||  */
        ...
        function trainAvailable(data){
            ...
            for(i=0;i<data.train.length;i++)
            {
                ...
                httpresponce += '<a href="#" class="trainFairList" name="'+ data.train[i].number +'" id="' + i + '">' + data.train[i].number + '</a>';
                httpresponce += '</td>';
...
require(["jquery","jquery-mloading"],function($){
$(document).on('click', '.trainFairList', function(e){
    var id = $(this).attr('id');
